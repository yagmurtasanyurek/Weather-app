    ...
    return {
      fetchWeather: builder.query({
        query: ({ lat, long }) => {
          ...
          return `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&daily=temperature_2m_max,temperature_2m_min,sunset,sunrise,weather_code,rain_sum,precipitation_probability_min,wind_speed_10m_min&hourly=temperature_2m,weather_code,rain,snowfall,relative_humidity_2m&models=best_match&current=temperature_2m,is_day,rain,snowfall,wind_speed_10m,precipitation,relative_humidity_2m&timezone=auto&timeformat=unixtime&start_date=${start}&end_date=${end}`;
        },
      }),
